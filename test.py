...
for key, entry in data.items():
    docs = []
    image_and_caption = []
    for txt_Facts in entry['txt_Facts']:
        fact = txt_Facts['fact']
        docs.append(fact)
    rag.insert(docs)
    for img_Facts in entry['img_Facts']:
...
print(
    rag.query("\"Is there smoke trailing from both the parachuter above Los Angeles at the 2005 Extreme Games and the paraglider in the Bucegi Mountains?\"", param=QueryParam(mode="hybrid"))
)
