...
class LightRAG:
    ...
    def __post_init__(self):
        ...
        # INFO:lightrag:Logger initialized for working directory: ./story
    ...
    # 把新的内容插入到图里面，防止重复工作
    # 同步包装
    def insertImage(self, image_and_caption, split_by_character=None):
        loop = always_get_an_event_loop()
        return loop.run_until_complete(
            self.ainsertImage(image_and_caption, split_by_character)
        )
    # 异步函数
    async def ainsertImage(self, image_and_caption, split_by_character):
        ...
        unique_contents = list(
            (doc["img"], doc["txt"].strip()) 
            for doc in image_and_caption 
            if "img" in doc and "txt" in doc
        )

        # 2. Generate document IDs and initial status
        new_docs = {
            # 键值对的形式，前面的函数生成一个MD5编码
            compute_mdhash_id(content[1], prefix="doc-"): {
                "content": content[1],
                "img":content[0],
                # 这个摘要只是前100个字符
                "content_summary": self._get_content_summary(content[1]),
        ...
        # 过滤掉已经处理过的文档，doc_status是一个存文档状态的仓库
                ...
                try:
                    ...
                    chunks = {
                        ...
                        for dp in chunking_by_token_size(
                            doc["content"],
                            split_by_character=split_by_character,  # 应该是也可以根据character分割，但是默认根据token分割
                            overlap_token_size=self.chunk_overlap_token_size,
                            max_token_size=self.chunk_token_size,
                            tiktoken_model=self.tiktoken_model_name,
                        )
                        # 这个chunking_by_token_size函数是把文档分成了多个chunk
                        # 返回的dp格式为：
                        #"""
                    ...
                    try:
                        ...
                        # 提取实体和关系，并把实体，实体关系图，关系等存进对应的数据库
    ...
    async def aquery(self, query: str, param: QueryParam = QueryParam()):
        if param.mode in ["local", "global", "hybrid"]:
            response = await kg_query(
                query,
                self.chunk_entity_relation_graph, # 图
                self.entities_vdb,                # 实体的向量形式
                self.relationships_vdb,           # 关系的向量形式
                self.text_chunks,                 # chunks的文本形式
                param,
                asdict(self),
                hashing_kv=self.llm_response_cache
                if self.llm_response_cache
            ...
            response = await naive_query(
                query,
                self.chunks_vdb, # chunks的向量形式
                self.text_chunks,# chunks的纯文本形式
                param,
                asdict(self),
                hashing_kv=self.llm_response_cache
                if self.llm_response_cache
