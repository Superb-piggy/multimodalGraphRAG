...
async def extract_entities(
    ...
    if(img is not None):
        for k,v in maybe_nodes.items():
            maybe_edges[("IMG"+img,k)].extend([{"src_id":"IMG"+img,"tgt_id":k,"weight":5.0,"description":f"The image contains {k}","keywords":"contains","source_id":compute_mdhash_id("IMG"+img,prefix="-")}])
        maybe_nodes["IMG"+img].extend([{"entity_name":"IMG"+img,"entity_type":"image","description":content,"source_id":compute_mdhash_id("IMG"+img,prefix="ent-")}])
    logger.info("Inserting entities into storage...")
    all_entities_data = []
    for result in tqdm_async(
        asyncio.as_completed(
            [
                _merge_nodes_then_upsert(k, v, knowledge_graph_inst, global_config) # 把点加到图里面吧，这个图实际上是kv数据库
                for k, v in maybe_nodes.items()
            ]
        ),
    ...
    if entity_vdb is not None:
        ...
        for dp in all_entities_data:
            if dp["entity_name"].startswith("IMG"):
                # 如果 entity_name 以 "IMG" 开头，则处理为新的格式
                img_part = dp["entity_name"][3:]  # 去掉 "IMG" 的部分
